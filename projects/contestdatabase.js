...
function participantHistory_graphgen() { //call with a button call
    ...
    //Now the key.
    phgg_svg += '<rect x="' + (phgg_svg_width - 96) + '" y="' + (phgg_svg_height/2 - 54) + '" height="8" width="8" fill="#66AAFF"></rect>';
    phgg_svg += '<text x="' + (phgg_svg_width - 80) + '" y="' + (phgg_svg_height/2 - 50) + '" font-family="Andale Mono, Monospace" font-size="14px" fill="#DDDDDD"' +
          'text-anchor="start" dominant-baseline="central">MotK</text>';
    phgg_svg += '<rect x="' + (phgg_svg_width - 96) + '" y="' + (phgg_svg_height/2 - 24) + '" height="8" width="8" fill="#66FFAA"></rect>';
    phgg_svg += '<text x="' + (phgg_svg_width - 80) + '" y="' + (phgg_svg_height/2 - 20) + '" font-family="Andale Mono, Monospace" font-size="14px" fill="#DDDDDD"' +
          'text-anchor="start" dominant-baseline="central">LOCAA</text>';
    ...
    for (i = 0; i < contests.length; i++) { //for each contest
        ...
        //Now create the SVG rectangle iff there is an end date
        if (cleared) {
            var phgg_basestart = ((phgg_start[0]-min)*12 + phgg_start[1])*16; //does not include 48 pixel left and 4 pixel buffer
