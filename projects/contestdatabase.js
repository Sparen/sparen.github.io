...
function contestsPerYear() { //WARNING: NUMBER OF LOCATIONS IS CURRENTLY HARDCODED
    ...
    //key
    cpy_svg += generatekey(cpy_svg_width, 80);
...
function participantsOverTime() { //WARNING: NUMBER OF LOCATIONS IS CURRENTLY HARDCODED
    ...
    let pot_svg_width = 168 + years.length * 16 * 12;
    ...
    for (yearctr = 0; yearctr < years.length; yearctr += 1) { //years
        for (monthctr = 0; monthctr < 12; monthctr += 1) {
            pot_svg += createCDBSVGText(48 + yearctr * 16 * 12 + monthctr * 16, 400 - 50, "cdb_verttext_a", months[monthctr] + " " + years[yearctr]);
        }
    }
...
function participantHistory_graphgen() { //call with a button call
    ...
    const NAME_SIDEBUFFER_LT = 144;
    // Left side gets away with names overflowing into a space without contests since first contest tracked is in October.
    // Right side overflows into key without larger size
    // Basically just long enough to fit Miransu's uh... list of names.
    const NAME_SIDEBUFFER_RT = 240;
    const KEY_SIDEBUFFER = 128;
    ...
    let phgg_svg_width = NAME_SIDEBUFFER_LT + years.length * 16 * 12 + NAME_SIDEBUFFER_RT + KEY_SIDEBUFFER;
    let phgg_svg_height = FOOTER_HEIGHT + 16 * phgg_selpartic.length;
    ...
    //Now let's plot the timestamps. The years are 16*12 pixels apart, with 16 pixels per month.
    //NAME_SIDEBUFFER_LT is the buffer on the left (for names). KEY_SIDEBUFFER is the buffer on the right (for the key)
    for (yearctr = 0; yearctr < years.length; yearctr += 1) { //years
        for (monthctr = 0; monthctr < 12; monthctr += 1) {
            phgg_svg += createCDBSVGText(NAME_SIDEBUFFER_LT + yearctr*16*12 + monthctr*16, (phgg_svg_height - 108), "cdb_verttext_a", months[monthctr] + " " + years[yearctr]);
        }
    }
    ...
    for (contestctr = 0; contestctr < contests.length; contestctr += 1) { //for each contest
        ...
        //Now create the SVG rectangle iff there is an end date
        if (cleared) {
            let phgg_basestart = ((phgg_start[0]-min)*12 + phgg_start[1])*16; //does not include NAME_SIDEBUFFER_LT pixel left and 4 pixel buffer
            let phgg_basewidth = ((phgg_end[0] - phgg_start[0])*12 + phgg_end[1] - phgg_start[1])*16; //does not include 8 pixel base rect width
            textlabels += '<text x="' + (NAME_SIDEBUFFER_LT + phgg_basestart + phgg_basewidth/2) + '" y="' + (phgg_svg_height - 80) + 
            ...
            textlabels += '<path d="M' + (NAME_SIDEBUFFER_LT + phgg_basestart + phgg_basewidth/2) + ' 4 V' + (phgg_svg_height - 156) + '" stroke-width="1" stroke="#333333"></path>'; //contest guideline
        }
    }
    ...
    for (participantctr = 0; participantctr < phgg_selpartic.length; participantctr += 1) { //for each and every player.
        ...
        let phgg_string = createCDBSVGText(8, phgg_curry, "cdb_horiztext_a", phgg_player) + createCDBSVGText(phgg_svg_width - KEY_SIDEBUFFER - NAME_SIDEBUFFER_RT, phgg_curry, "cdb_horiztext_a", phgg_player); // As of Aug 17, 2025, adding another one to the RHS since it's to hard to scroll all the way left now
        let phgg_startendpath = '<path d="M 4 ' + (phgg_curry - 8) + ' H ' + (phgg_svg_width - KEY_SIDEBUFFER) + '" stroke-width="1" stroke="#333333"></path>'; //default guide lines
        phgg_startendpath = phgg_startendpath + '<path d="M 4 ' + (phgg_curry + 8) + ' H ' + (phgg_svg_width - KEY_SIDEBUFFER) + '" stroke-width="1" stroke="#333333"></path>'; //default guide lines
        let phgg_nodes = "";
        ...
        for (contestctr = 0; contestctr < contests.length; contestctr += 1) { //for each contest
            ...
            let phgg_basestart = ((phggindiv_start[0] - contests[0].year)*12 + phggindiv_start[1])*16; //does not include NAME_SIDEBUFFER_LT pixel left and 4 pixel buffer
            ...
            let nodelocation = NAME_SIDEBUFFER_LT + phgg_basestart + phgg_basewidth/2;
