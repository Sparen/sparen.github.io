...
function Single_1() {
    ...
    tasks.push(new Single_1_Task_SpawnPRing(["lightcoral", "blanchedalmond", "lightseagreen", "chartreuse"][Math.floor((Math.random() * 4))]));
...
function Single_1_Task_SpawnPRing(color) {
    this.counter = 0;
    ...
    this.update = function () {
        ...
        if (everyinterval(1) && this.counter < this.maxcounter) { 
            var i;
            for (i = 0; i < 3; i++) {
                var newshot = new EnemyShot(192 + 120*Math.cos(this.angleT * 3), 224 + 90*Math.sin(this.angleT * 5), 0, this.angleT * 3, 0.0015, 5, color, 3, 5, 0.75, 4, -1);
